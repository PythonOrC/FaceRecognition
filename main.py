...
class DoorUnlockSystem:
    ...
    def __init__(self, camera_index: int = None):
        ...
        print(f"Database has {len(self.database.get_all_persons())} registered persons")
    ...
    def _draw_status(self, frame: np.ndarray, status: str, color: tuple = (255, 255, 255)):
    ...
    def process_single_frame(self, frame: np.ndarray) -> tuple[bool, str, Optional[str]]:
        """
        ...
        if self.consecutive_good_frames < config.CONSECUTIVE_GOOD_FRAMES:
            return False, f"Stabilizing... ({self.consecutive_good_frames}/{config.CONSECUTIVE_GOOD_FRAMES})", None
        ...
        if match.distance > config.MATCH_THRESHOLD:
            return False, f"Match below threshold ({match.distance:.3f})", match.person_name
        
        if match.margin is not None and match.margin < config.MATCH_MARGIN:
            return False, f"Match margin too small ({match.margin:.3f})", match.person_name
        
        # ═══════════════════════════════════════════════════════════════
        # STEP 5: Anti-Replay / Anti-Reuse Check
        # ═══════════════════════════════════════════════════════════════
        anti_result = self.anti_replay.check(frame, embedding, match.person_name)
        
        if not anti_result.passed:
            return False, f"Security check failed: {anti_result.message}", match.person_name
        ...
        self.database.add_embedding_to_matched_person(match.person_name, embedding)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Face Recognition Door Unlock System')
    parser.add_argument('--camera', '-c', type=int, default=config.CAMERA_INDEX,
                        help=f'Camera index (default: {config.CAMERA_INDEX})')
    parser.add_argument('--list-persons', '-l', action='store_true',
                        help='List all registered persons and exit')
    parser.add_argument('--register', '-r', type=str, metavar='NAME',
    ...
    if args.list_persons:
        ...
        if persons:
            ...
            for name in sorted(persons):
                ...
                if info.last_seen:
                    print(f"    Last seen: {info.last_seen.strftime('%Y-%m-%d %H:%M')}")
        ...
        try:
            while True:
                ...
                if detection.success:
                    draw_face_box(display, detection.face_data.location, "Ready")
                    cv2.putText(display, "Press SPACE to register", (10, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                else:
                    cv2.putText(display, detection.message, (10, 30),
                ...
                if key == ord(' ') and detection.success:
                    system.database.register_person(args.register, detection.face_data.embedding)
